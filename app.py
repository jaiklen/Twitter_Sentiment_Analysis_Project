...
matplotlib.use('Agg')
nltk.download('stopwords')  ###downloading important liberary 
...
def func_gtweet(topic_name,typ):
    tweetList = []
    userList = []
    likesList = []
    datetimeList = []
    locationList = []
    if(typ == 'person'):
        cursor = api.user_timeline(screen_name= topic_name,count=200,include_rts = False,tweet_mode = 'extended')
    else:
        Query = topic_name + "-filter:retweets"
        cursor = tweepy.Cursor(api.search_tweets, q=Query, lang="en",tweet_mode="extended", exclude="retweets").items(200)
    for t in cursor:
    ...
    df = pd.DataFrame({"User Name": userList, "Tweets": tweetList, "Likes": likesList,"Date Time": datetimeList, "Location": locationList})
    return df
...
###################################################
##function for cleaning and finding sentiment of tweets
####################################################
def polarity_dataframe(data_frame):
    tempdf = data_frame
    x = tempdf["Tweets"]
    ...
    def sentiment_analysis(data_frame):
        ...
        data_frame["Subjectivity"] = data_frame["Clean Tweets"].apply(getSubjectivity)
        data_frame["Polarity"] = data_frame["Clean Tweets"].apply(getPolarity)
        def getAnalysis(score):
            if score <= -0.1:
                ...
                return "positive"
        data_frame["Polarity_Analysis"] = data_frame["Polarity"].apply(getAnalysis)
        return data_frame
    newdf = sentiment_analysis(tempdf)
    return newdf

###################################################
#####3A function to plot pie chart and bar graph
###################################################
def helper_plot(topic_name,Analysis_type, typ='movies'):
    df1 =func_gtweet(topic=topic_name,typ= typ)
    newdf = polarity_dataframe(df1)
    ...
    if Analysis_type== "Pie Chart":
        x = {'Positive': posNumber,'Negative': negNumber,'Neutral': neuNumber,}
        data = pd.Series(x).reset_index(name='value').rename(columns={'index': 'country'})
        data['angle'] = data['value']/data['value'].sum() * 2*pi
        data['color'] = ["seagreen", "#ffa600", "#58508d"]
        output_file("plot.html")
        p = figure(height=400,width=700, title=f"Pie Chart for sentiments analysis  of latest {total} tweets", toolbar_location=None,tools="hover", tooltips="@country: @value", x_range=(-0.5, 1.0))
        ...
        p = figure(height=500,width=800,x_range=sentiments, title=f"Bar Graph for sentiments analysis  of latest {total} tweets",toolbar_location=None, tools="hover",tooltips="@sentiments: @counts", x_axis_label="Sentiments", y_axis_label="Tweets Count")
...
def subject():
    if(request.method == 'POST'):
        try:
            ...
            pola_df = polarity_dataframe(func_gtweet(subject_name,'subject'))
            global final_n,final_p
            final_p,final_n = helper(pola_df)
            p= helper_plot(str(subject_name), str(p_type),'subject')
...
def personal():
    ...
    if(request.method == 'POST'):
        try:
            ...
            pola_df = polarity_dataframe(func_gtweet(person_id,'person'))
            global final_n,final_p
            final_p, final_n = helper(pola_df)
            p = helper_plot(str(person_id), str(result_type), 'person')
            demo_script_code, chart_code = components(p)
            return render_template('result.html', chart_code=chart_code, demo_script_code=demo_script_code, titles=final_p.columns.values)
        except:
            return render_template('error.html')
...
#####################################################
def helper(polarity_dataf):
    # Wordclod for mixed tweets
    all_tweets = []
    for i in range(len(polarity_dataf)):
        all_tweets.append(polarity_dataf.loc[i].Tweets)
    ...
    neg = []
    for i in range(len(polarity_dataf)):
        if polarity_dataf.loc[i].Polarity_Analysis == "positive":
            pos.append(polarity_dataf.loc[i].Tweets)
        elif polarity_dataf.loc[i].Polarity_Analysis == "negative":
            neg.append(polarity_dataf.loc[i].Tweets)

    path_static = os.path.join(os.getcwd(), "static/")

    # Wordcloud for +ve Tweets
    text_pp = ' '.join(pos)
    word_cloud_p_processed = WordCloud(collocations=False, background_color='white').generate(proc(text_pp))  
    plt.imshow(word_cloud_p_processed, interpolation='bilinear')
    plt.axis("off")
    ...
    text_nn = ' '.join(neg)
    word_cloud_p_processed = WordCloud(collocations=False, background_color='white').generate(proc(text_nn)) 
    plt.imshow(word_cloud_p_processed, interpolation='bilinear')
    plt.axis("off")
    plt.tight_layout(pad=0.1)
    ...
    for i in range(len(polarity_dataf)):
        if polarity_dataf.loc[i].Polarity_Analysis == "positive":
            top_pos_10.append(polarity_dataf.loc[i])
    ...
    top_neg_10 = []
    for i in range(len(polarity_dataf)):
        if polarity_dataf.loc[i].Polarity_Analysis == "negative":
            top_neg_10.append(polarity_dataf.loc[i])
...
def movie():
    if(request.method == 'POST'):
        try:
            ...
            print(tmp[1])
            if(tmp[0] == True):
                pola_df =polarity_dataframe(func_gtweet(tmp[1],'movie'))
                global final_n,final_p
                final_p, final_n = helper(pola_df)
                p=helper_plot(str(tmp[1]), str(result_type),'movie')
                demo_script_code, chart_code = components(p)
                return render_template('result.html', chart_code=chart_code, demo_script_code=demo_script_code,titles=final_p.columns.values)
            else:
                return render_template('error.html')
...
@app.route('/neg')
def negative():
    return render_template('negative.html',table1=[final_n.to_html(classes='data1', index=False)],titles=final_n.columns.values)
